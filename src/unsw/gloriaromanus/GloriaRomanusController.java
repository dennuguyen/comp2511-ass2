...
public class GloriaRomanusController {
  ...
  @FXML
  public void initData(String initMode, int currentPlayer, boolean isEnded, World world, Victory victory, List<Faction> players)
    ...
    // Attach faction to turn as observers
    for (Faction player : players) {
      turn.attach(player);
    }
  ...
  public void clickedSaveGame(ActionEvent e) throws IOException {
    if (!isEnded) {
      PersistanceFactory.writeToFile(currentPlayer, victory, players, "NO");
    } else {
  ...
  private void changeCurrentPlayerOnScreen() throws IOException {
    if (controllerParentPairs.get(0).getKey() instanceof SideMenuController) {
      ((SideMenuController) controllerParentPairs.get(0).getKey()).setCurrentFaction(players.get(currentPlayer));
    }
  }
  ...
  public void setTaxLevel(Province province, TaxLevel taxLevel) throws IOException {
    ...
    if (controllerParentPairs.get(0).getKey() instanceof ProvinceMenuController) {
      ((ProvinceMenuController) controllerParentPairs.get(0).getKey())
          .setProvince(provinceToOwningFactionMap.get(province.getLocation()), province, true);
    }
  }
}
